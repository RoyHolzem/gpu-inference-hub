...
import { consumerGPUs, modelRequirements } from '@/lib/gpu-data'
  ...
  return (
    <main className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold">Personal Use</h1>
                <p className="text-xs text-gray-500">Consumer GPUs (RTX Series)</p>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-4">
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex flex-col md:flex-row gap-4 mb-8">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-surface border border-border rounded-lg pl-10 pr-4 py-3 focus:outline-none focus:border-accent-green transition-colors"
            />
          </div>
          <div className="flex gap-2">
            <button
              ...
              className={`px-4 py-2 rounded-lg transition-colors ${
                sortBy === 'vram' ? 'bg-accent-green text-black' : 'bg-surface border border-border'
              }`}
            >
              VRAM
            </button>
            <button
              ...
              className={`px-4 py-2 rounded-lg transition-colors ${
                sortBy === 'performance' ? 'bg-accent-green text-black' : 'bg-surface border border-border'
              }`}
            >
              Performance
            </button>
        ...
        >
          <h3 className="text-lg font-semibold mb-4">Model Requirements Reference</h3>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="text-left py-2 text-gray-400">Min GPU</th>
                </tr>
              </thead>
              <tbody>
                {modelRequirements.slice(0, 3).map((model) => (
                  <tr key={model.name} className="border-b border-border/50">
                    ...
                    <td className="py-2 text-accent-green">{model.vram_fp16} GB</td>
                    <td className="py-2 text-yellow-500">{model.vram_int4} GB</td>
                    <td className="py-2 text-gray-300">{model.minGPU}</td>
                  </tr>
                ))}
              </tbody>
            </table>
